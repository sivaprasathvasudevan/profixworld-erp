...
const CACHE = 'profix-store-v13';
const SHELL = ['/', '/index.html', '/offline.html', '/manifest.webmanifest', '/icon-192.png', '/icon-512.png', '/profix-scan.js'];
  ...
  if (req.mode === 'navigate') {
    e.respondWith(
      fetch(req)
        ...
        .catch(() => caches.match(req).then(m => m || caches.match('/').then(r => r || caches.match('/offline.html'))))
    );
    return;
  }

  // Manifests: always network-first. A cached manifest freezes the app's identity
  // (id/scope), so Chrome keeps seeing the old one and refuses to install the app.
